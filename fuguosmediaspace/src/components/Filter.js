...
function Filter() {
  ...
  useEffect(() => {
    if (activeCat === "All") {
      ...
      setFiltered(all.filter(project => Capitalize(project.category) === activeCat));
    }
  }, [activeCat, all]);
  ...
  const extractAndSortDistinctValues = (data) => {
    ...
    const sortedValues = distinctCategories.sort((a, b) => {
      if (a === "All") return -1;
      if (b === "All") return 1;
      return (categoryPriority.indexOf(a) || Infinity) - (categoryPriority.indexOf(b) || Infinity);
  ...
  const categoryList = distinctCategories.map((category) => (
    <li className="me-4 md:me-6 text-gray-900">
      <button className="capitalize hover:underline" onClick={()=>setActiveCat(category)} key={categoryPriority.indexOf(category)}>{category}</button>
    </li>
  ));
